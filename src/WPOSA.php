...
class WPOSA {
	 ...
	 * @return void
	 */
	function callback_html( $args ): void {
	...
	function callback_file( $args ) {
		...
		$label = $args['options']['button_label'] ?? __( 'Choose File', 'mytracker' );
	...
	function callback_image( $args ) {
		...
		$size  = isset( $args['size'] ) && ! is_null( $args['size'] ) ? $args['size'] : 'regular';
		$id    = $args['section'] . '[' . $args['id'] . ']';
		$label = $args['options']['button_label'] ?? __( 'Choose Image', 'mytracker' );
	...
	function script() {
		...
		<script>
			jQuery( document ).ready( function( $ ) {
				...
				$help.on(
					...
					function () {
						const $this = $(this);
						const tab = '#tab-link-<?php echo esc_js( Utils::get_plugin_prefix() ); ?>_' + $this.data('tab');
		...
		<style>
			...
			.wposa-form-table__row_type_select .regular {
				width: 150px;
			}
